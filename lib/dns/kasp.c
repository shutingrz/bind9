...
dns_kasp_create(isc_mem_t *mctx, const char *name, dns_kasp_t **kaspp)
{
	...
	kasp->dnskey_ttl = DNS_KASP_DNSKEY_TTL;
...
dns_kasp_signdelay(dns_kasp_t *kasp) {
	...
	validity = kasp->signatures_validity;
	if (kasp->signatures_validity_denial > validity) {
		validity = kasp->signatures_validity_denial;
	}
	return (validity - kasp->signatures_refresh);
}
