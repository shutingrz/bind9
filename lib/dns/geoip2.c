...
ISC_THREAD_LOCAL geoip_state_t *geoip_state = NULL;

static isc_result_t
set_state(isc_mem_t *mctx, const MMDB_s *db, const isc_netaddr_t *addr,
	  MMDB_lookup_result_s mmresult, MMDB_entry_s entry,
	  geoip_state_t **statep)
{
	if (geoip_state == NULL) {
		geoip_state = isc_mem_get(mctx, sizeof(*geoip_state));
		*geoip_state = (geoip_state_t){ 0 };
	}
...
get_entry_for(isc_mem_t *mctx, MMDB_s * const db, const isc_netaddr_t *addr) {
	isc_result_t result;
	isc_sockaddr_t sa;
	...
	result = set_state(mctx, db, addr, match, match.entry, &geoip_state);
		...
		const dns_geoip_elem_t *elt)
{
	...
	state = get_entry_for(geoip->mctx, db, reqaddr);
	if (state == NULL) {
		return (false);
	}
