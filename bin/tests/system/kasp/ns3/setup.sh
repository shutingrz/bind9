...
# so:   Tact(N) = Tdea(N) - Lksk - Iret = now - 60d - 3d2h = now - 1514h
# and:  Tret(N) = Tdea(N) - Iret = now - 3d2h = 74h
# and:  Tpub(N+1) = Tdea(N) - Iret - Dreg - IpubC = now - 74h - 1d - 1d3h = now - 125h
# and:  Tsbm(N+1) = Tdea(N) - Iret - Dreg = now - 74h - 1d = now - 98h
# and:  Tact(N+1) = Tret(N)
# and:  Tret(N+1) = Tdea(N) + Lksk - Iret = now + 60d - 3d2h = now + 1366h
TactN="now-1514h"
TretN="now-74h"
TpubN1="now-125h"
TsbmN1="now-98h"
...
$SETTIME -s -S $KSK1              -i 0                                                      $KSK2 > settime.out.$zone.3 2>&1
$SETTIME -s -P $TpubN1 -A $TactN1           -g $O -k $O $TactN1 -r $O $TactN1 -d $O $TactN1 $KSK2 > settime.out.$zone.1 2>&1
$SETTIME -s -P $TactN  -A $TactN            -g $O -k $O $TactN  -z $O $TactN                $ZSK  > settime.out.$zone.2 2>&1
cat template.db.in "${KSK}.key" "${ZSK1}.key" "${ZSK2}.key" > "$infile"
private_type_record $zone 13 $KSK1 >> "$infile"
private_type_record $zone 13 $KSK2 >> "$infile"
